...
- `hasProject == false`    → 同上
- `editorListening == false`：
  - 先用 `run_command` 执行 godot-deploy 的步骤 5 脚本（[见步骤 5](#步骤-5-自动启动-godot-并启用-mcp-插件)），自动启动 Godot + 启用插件
  - 只有步骤 5 返回 `[FAIL]` 找不到编辑器时才让位给 godot-deploy 完整部署
